...
class Details extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            method: 'GET',
            url: `http://localhost:4475/serie/${serie}`,
            headers: { 'Content-Type': 'application/json' }
        })
            .then(response => {
                this.setState({ serie: response.data.serie })
            })
            .catch()
    }
    ...
    handleOrder = (resId) => {
        axios({
            method: 'GET',
            url: `http://localhost:4475/menuitems/${resId}`,
            headers: { 'Content-Type': 'application/json' }
        })
            .then(response => {
    ...
    getData = (data) => {
        return fetch(`http://localhost:4475/payment`, {
            method: "POST",
            headers: {
    ...
    render() {
        const { serie, menuItems, menuItemsModalIsOpen, galleryModalIsOpen, formModalIsOpen, subTotal } = this.state;
        return (
            <div>
                <div>
                    <img src={`./${serie.image}`} alt="No Image, Sorry for the Inconvinience" width="100%" height="300px" />

                    <button className="button" onClick={() => this.handleModal('galleryModalIsOpen', true)}>Click to see Image Gallery</button>
                </div>
                <div className="heading">{serie.name}</div>
                <button className="btn-order" onClick={() => this.handleOrder(serie._id)}>Place Online Order</button>
                ...
                <div className="tabs">
                    <div className="tab">
                        ...
                        <div className="content">
                            <div className="about">About this Model</div>
                            <div className="head">Features</div>
                            <div className="value">{serie && serie.speciality && serie.speciality.map((item) => { return `${item.name}, ` })} </div>
                            <div className="head">Starting Cost</div>
                            <div className="value">&#8377; {serie.min_price}</div>
                        </div>
                    </div>
                    ...
                    <div className="tab">
                        ...
                        <div className="content">
                            <div className="head">Special Feature</div>
                            <div className="value">{serie.special_feature}</div>
                            <div className="head">Battery</div>
                            <div className="value">{serie.battery}</div>
                            <div className="head">Font camera</div>
                            <div className="value">{serie.camera}</div>
                        </div>
                    </div>
                </div>
                <Modal
                ...
                >
                    <div>
                        ...
                        <div >
                            <h3 className="restaurant-name">{serie.name}</h3>
                ...
                >
                    <div>
                        ...
                        <Carousel
                            ...
                            {serie && serie.thumb && serie.thumb.map(item => {
                                return <div>
